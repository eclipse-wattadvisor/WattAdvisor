...
class ThermalEnergyStorage(InvestmentComponent):
    ...
    def _add_variables(self, model: pyoe.Model, t: pyoe.RangeSet) -> pyoe.Model:
        ...
        self._input_thermal = pyoe.Var(t)
        model.add_component("{}_input_thermal".format(self.name), self._input_thermal)
        ...
        self.bilance_variables.input[EnergyType.THERMAL] = self._input_thermal
    ...
    def _add_constraints(self, model: pyoe.Model, t: pyoe.RangeSet) -> pyoe.Model:
        ...
        for tx in t:
            self._eq02.add(
                self._input_thermal[tx] == self._chg[tx] - self._dc[tx] * self.eff
            )
