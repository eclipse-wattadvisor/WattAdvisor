...
class ElectrodeBoiler(InvestmentComponent):
    ...
    eff : float
        Efficiency grade of the boiler [kW heat per kW electricity]
    """
    ...
    def _add_constraints(self, model: pyoe.Model, t: pyoe.RangeSet) -> pyoe.Model:
        ...
        for tx in t:
            self._eq02.add(
                self._input_electrical[tx] * self.eff == self._output_thermal[tx]
            )
