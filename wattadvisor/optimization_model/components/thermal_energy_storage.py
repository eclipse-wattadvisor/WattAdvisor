...
class ThermalEnergyStorage(Component):
    ...
    def __init__(self, 
        ...
        installed_capacity : float
            Already installed thermal storage capacity of the component [kWh] which acts as a lower bound
        installed_power : float | None, optional
            Already installed thermal power of the component [kW] which acts as a lower bound, by default None
        ...
        capex_power : float | None, optional
            Capital expenditure cost part of the component per thermal power [€/kW], by default None
        capex_capacity : float | None, optional
            Capital expenditure cost part of the component per thermal energy storage [€/kWh], by default None
    ...
    def _add_variables(self, model: pyoe.Model, t: pyoe.RangeSet) -> pyoe.Model:
        ...
        # binary values, to stop simultaneous charging and discharging
        # self.bin=pyoe.Var(t, domain=pyoe.Boolean)
        # model.add_component('{}_bin'.format(self.name), self.bin)

        # self.bout=pyoe.Var(t, domain=pyoe.Boolean)
        # model.add_component('{}_bout'.format(self.name), self.bout)
        ...
        self.bilance_variables.input[enums.EnergyType.THERMAL] = self.balance
    ...
    def _add_constraints(self, model: pyoe.Model, t: pyoe.RangeSet) -> pyoe.Model:
        ...
        for tx in t:
            if tx == 1:
                self.eq03.add(self.storage[tx] == self.chg[tx] - self.dc[tx])
            else:
